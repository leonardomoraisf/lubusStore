<!-- Google Font: Source Sans Pro -->
<link rel="stylesheet" href="{{URL}}/resources/assets/fonts/font_sans.css">
<!-- Font Awesome -->
<link rel="stylesheet" href="{{URL}}/resources/assets/admin/plugins/fontawesome-free/css/all.min.css">
<!-- Ionicons -->
<link rel="stylesheet" href="https://code.ionicframework.com/ionicons/2.0.1/css/ionicons.min.css">
<!-- Tempusdominus Bootstrap 4 -->
<link rel="stylesheet" href="{{URL}}/resources/assets/admin/plugins/tempusdominus-bootstrap-4/css/tempusdominus-bootstrap-4.min.css">
<!-- iCheck -->
<link rel="stylesheet" href="{{URL}}/resources/assets/admin/plugins/icheck-bootstrap/icheck-bootstrap.min.css">
<!-- JQVMap -->
<link rel="stylesheet" href="{{URL}}/resources/assets/admin/plugins/jqvmap/jqvmap.min.css">
<!-- Theme style -->
<link rel="stylesheet" href="{{URL}}/resources/assets/admin/dist/css/adminlte.min.css">
<!-- overlayScrollbars -->
<link rel="stylesheet" href="{{URL}}/resources/assets/admin/plugins/overlayScrollbars/css/OverlayScrollbars.min.css">
<!-- Daterange picker -->
<link rel="stylesheet" href="{{URL}}/resources/assets/admin/plugins/daterangepicker/daterangepicker.css">
<!-- summernote -->
<link rel="stylesheet" href="{{URL}}/resources/views/assets/admin/plugins/summernote/summernote-bs4.min.css">
<!--categories-->
<link rel="stylesheet" href="{{URL}}/resources/views/admin/styles/views.css">
